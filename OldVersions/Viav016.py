...
class PARC:
    ...
    def path_length_onbias(self, bias_edgelist, bias_edgeweights):
        ...
        #bias_edgelist = np.asarray(bias_edgelist)
        #bias_edgeweights = np.asarray(bias_edgeweights)
        #th = np.mean(bias_edgeweights[bias_edgeweights>np.percentile(bias_edgeweights,5)])#-np.std(bias_edgeweights[bias_edgeweights>np.percentile(bias_edgeweights,5)])
        #print('th', th)
        #loc_keep = np.where(bias_edgeweights > th)
        #bias_edgeweights = bias_edgeweights[loc_keep]
        #bias_edgelist = bias_edgelist[loc_keep]
    ...
    def simulate_markov(self, A):
        ...
        for i in range(num_sim):
            ...
            if i % 100 == 0: print(i, 'th simulation in Markov', time.ctime())
            state = state_root
            currentState = root
            stateHist = state
            ...
            if i % 100 == 0: print('hitting temp', hitting_array_temp.flatten())
        hitting_array = hitting_array[:, 1:]

        for i in range(n_states):
            rowtemp = hitting_array[i, :]
            if np.sum(rowtemp != (n_steps + 1)) != 0:
                perc = np.percentile(rowtemp[rowtemp != n_steps+1],5)
            ...
            for e_i, (start, end) in enumerate(edgelist):
                ...
                if direction_arrow == 1:
                    ...
                    ax_i.arrow(xp[250], smooth[250], xp[250 - step] - xp[250],
                              smooth[250 - step] - smooth[250], shape='full', lw=0,
                              length_includes_head=True, head_width=arrow_head_w, color='black')
                ax_i.scatter(node_pos[:, 0], node_pos[:, 1], s=group_pop*0.7, c=pt, cmap='viridis_r',
                             alpha=1, zorder=3)
...
def main():
    ...
    start_cell = 'C10'#C108 for M12 connected' #M8n1000d1000 start - c107 #c1001 for bifurc n2000d1000 #disconnected n1000 c108, "C1 for M10 connected"
