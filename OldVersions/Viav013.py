...
def get_biased_weights(edgelist, weights, pt):
    ...
    C = 1
    nu = 0.8
    upper_lim =  np.percentile(weights, 80)
    lower_lim = np.percentile(weights,20)
...
def local_pruning_clustergraph(adjacency_matrix, local_pruning_std = 0.0, global_pruning_std =2):
    ...
    for i in range(neighbor_array.shape[0]):
        ...
        to_keep_index = np.where(row > np.percentile(row, 30))[0]#-local_pruning_std*np.mean(row))[0] #we take [1] because row is a 2D matrix, not a 1D matrix like in other cases  # 0*std
...
class PARC:
    ...
    def run_subPARC(self):
        ...
        if self.pseudotime == True:
            ...
            hitting_times_2, roundtrip_times_2 = self.compute_hitting_time_onbias(
                norm_lap,d_sqrt_inv, root=root, x_lazy=x_lazy,
            ...
            self.single_cell_pt_bias= self.project_hittingtimes_sc(hitting_times_2)
        ...
        for e_i, (start, end) in enumerate(edgelist):
            ...
            if direction_arrow == 1:
                ...
                ax2.arrow(xp[250], smooth[250], xp[250 - step] - xp[250],
                          smooth[250 - step] - smooth[250], shape='full', lw=0,
                          length_includes_head=True, head_width=arrow_head_w, color='black')
        ax2.scatter(node_pos[:, 0], node_pos[:, 1],  s=group_pop*8, c=pt, cmap='viridis_r', alpha = 1, zorder =3)
...
def main():
    ...
    p0 = PARC(adata_counts.obsm['X_pca'][:,0:20], true_label, jac_std_global=2, knn=30, too_big_factor=0.4, pseudotime=True,path = "/home/shobi/Trajectory/Datasets/"+dataset+"/", root = 3, root_str = root_str) #*.4
    p0.run_PARC()
    super_labels = p0.labels

    super_edges = p0.edgelist
    super_pt = p0.scaled_hitting_times #pseudotime pt
    p1 = PARC(adata_counts.obsm['X_pca'][:,0:20], true_label, jac_std_global=1, knn=30, too_big_factor=0.05, path = "/home/shobi/Trajectory/Datasets/"+dataset+"/",pseudotime=True, root =61, super_cluster_labels=super_labels, super_node_degree_list=p0.node_degree_list,root_str = root_str) #*.4
    p1.run_PARC()
    labels = p1.labels
    ...
    print('end tsne')
    draw_trajectory_dimred(embedding, labels, super_labels, super_edges,p1.x_lazy, p1.alpha_teleport, p1.single_cell_pt, true_label,knn=p0.knn, title_str='original hitting times')

    draw_trajectory_dimred(embedding, labels, super_labels, super_edges,
                           p1.x_lazy, p1.alpha_teleport, p1.single_cell_pt_bias, true_label, knn=p0.knn, title_str='biased hitting times')

    draw_trajectory_dimred(embedding, labels, super_labels, super_edges,
                           p1.x_lazy, p1.alpha_teleport, p1.single_cell_pt_dijkstra_bias, true_label, knn=p0.knn,
                           title_str='dijkstra on biased hitting')
