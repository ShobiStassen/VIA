...
#for EB data, changed the "pseudotime too low" to 20, instead of 30 percentile
...
def draw_trajectory_gams(X_dimred, sc_supercluster_nn, cluster_labels, super_cluster_labels, super_edgelist, x_lazy,
    ...
    for color, group in zip(line, sorted(set(true_label))):
        ...
        ax1.scatter(X_dimred[where, 0], X_dimred[where, 1], label=group, c=np.asarray(plt.cm.jet(color)).reshape(-1, 4), alpha = 0.5, s=5)
...
class PARC:
    ...
    def get_terminal_clusters(self, A, markov_pt, root_ai):
        ...
        for terminal_i in terminal_org:
            if terminal_i in terminal_clusters: removed_terminal_i = False
            else: removed_terminal_i = True
            ...
            if neigh_terminal.size > 0:
                ...
                if count_nn >= 3:
                    ...
                    print('TS', terminal_i, 'had 3 or more neighboring terminal states', ts_neigh,' and so we removed,', temp_remove)
    ...
    def run_subPARC(self):
        ...
        if self.is_coarse == True:
            ...
            G = ig.Graph(n = X_data.shape[0],edges = edgelist, edge_attrs={'weight': csr_array_locally_pruned.data.tolist()})  # used for PARC
            ...
            for comp_i in range(n_components):
                ...
                for target_terminal in terminal_clus_ai:
                    ...
                    prob_ai = self.simulate_branch_probability(target_terminal, terminal_clus_ai, adjacency_matrix2_ai,
                                                                new_root_index, pt=markov_hitting_times_ai, num_sim=50) #50 ToDO change back to 500 = numsim
...
def main_Bcell(ncomps, knn, random_seed):
    ...
    embedding = umap.UMAP(random_state=42, n_neighbors=15, init='random').fit_transform(        adata_counts.obsm['X_pca'][:, 0:5])
...
def main_iPSC(ncomps=100, knn=30, p0_random_seed=12, run_palantir_func=False):
    ...
    true_label_str = ['0'+str(i) if i<10 else str(i) for i in true_label]#the way find_root works is to match any part of root-user to majority truth
    ...
    p0.run_PARC()
    marker_genes = ['Oct4','Sox2','Klf4','c-Myc','Nanog','GFP','Lin28', 'SSEA1', 'EpCAM', 'CD54', 'CD73', 'Thy1', 'MEFSK4', 'CD140a', 'CD24', 'CD44', 'H3K9ac','Ki67']
    adata_idx = adata[idx]
...
def main_EB():
    ...
    marker_genes_dict0 = {'Hermang':['TAL1', 'HOXB4', 'SOX17','CD34', 'PECAM1'],'NS':['KLF7', 'ISL1', 'DLX1','ONECUT1','ONECUT2','OLIG1', 'NPAS1','LHX2', 'NR2F1','NPAS1','DMRT3', 'LMX1A','NKX2-8', 'EN2','SOX1','PAX6', 'ZBTB16'],'NC':['PAX3','FOXD3','SOX9','SOX10'],'EN':['ARID3A', 'GATA3','SATB1','SOX15','SOX17', 'FOXA2'],'PostEn':['CDX2', 'ASCL2', 'KLF5','NKX2-1'],'ESC':['NANOG', 'POU5F1','OTX2'],'Pre-NE':['GBX2', 'OLIG3','HOXD1','ZIC2','ZIC5','HOXA2','HOXB2' ],
                         'SMP':['TBX18', 'SIX2','TBX15', 'PDGFRA'],'Cardiac':['TNNT2','HAND1','F3','CD82','LIFR'],'EpiCard':['WT1'],'ME':['T'],'NE':['GBX2','ZIC2','ZIC5','GLI3','LHX2','LHX5','SIX3','SIX6']}

    marker_genes_dict=    {'Hermang': ['TAL1', 'HOXB4', 'SOX17', 'CD34', 'PECAM1'],
     'NS': ['KLF7', 'ISL1', 'DLX1', 'ONECUT1', 'ONECUT2', 'OLIG1', 'NPAS1', 'LHX2', 'NR2F1', 'NPAS1', 'DMRT3', 'LMX1A',
            'NKX2-8', 'EN2', 'SOX1', 'PAX6', 'ZBTB16'], 'NC': ['PAX3', 'FOXD3', 'SOX9', 'SOX10'],
     'PostEn': ['CDX2', 'ASCL2', 'KLF5', 'NKX2-1'],

     'SMP': ['TBX18', 'SIX2', 'TBX15', 'PDGFRA'], 'Cardiac': ['TNNT2', 'HAND1', 'F3', 'CD82', 'LIFR'],
     'EpiCard': ['WT1','TBX5', 'HOXD9', 'MYC'], 'ME': ['T'], 'NE': ['GBX2', 'ZIC2', 'ZIC5', 'GLI3', 'LHX2', 'LHX5', 'SIX3', 'SIX6']}
    marker_genes_list=[]
    ...
    knn = 30
    n_var_genes = 15000
    print('ncomps, knn, n_var_genes, p0big, p1big, randomseed, time', ncomps, knn, n_var_genes, p0_too_big, p1_too_big, p0_random_seed, time.ctime())
    #data = pd.read_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/EBT_counts_nosqrt_noNorm.csv')

    #data = data.drop(['Unnamed: 0'], axis=1)
    #print(data.shape, data)
    TI_pcs = pd.read_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/PCA_TI_200_TuesAM.csv')
    TI_pcs = TI_pcs.values[:,1:]

    umap_pcs = pd.read_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/PCA_umap_200_TuesAM.csv')
    umap_pcs = umap_pcs.values[:, 1:]
    #print('TI PC shape', TI_pcs.shape)
    from scipy.io import loadmat
    annots = loadmat('/home/shobi/Trajectory/Datasets/PhatescRNA/EBdata.mat') #has been filtered but not yet normed (by library s
    ...
    #time_labels = pd.read_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/labels_1.csv')
    ...
    #filter_result = sc.pp.filter_genes_dispersion(  # select highly-variable genes   adata.X, flavor='cell_ranger', n_top_genes=15000, log=False) #dont take log
    ...
    '''
    pcs = pd.read_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/umap_200_matlab.csv')
    ...
    #df_pca_TI_200.to_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/PCA_TI_200_TuesAM.csv')

    #df_pca_umap_200 = pd.DataFrame(adata_umap.obsm['X_pca'])
    #df_pca_umap_200.to_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/PCA_umap_200_TuesAM.csv')
    ...
    U = pd.read_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/umap_pc10_knn10.csv')
    U = U.values[:, 1:]

    #U = umap.UMAP(n_neighbors=u_knn, random_state=0, repulsion_strength=repulsion_strength).fit_transform(adata_umap.obsm['X_pca'][:, 0:n_pcs])
    ...
    #df_U = pd.DataFrame(U)
    #df_U.to_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/umap_pc10_knn10.csv')
    ...
    p0_labels_df.to_csv('/home/shobi/Trajectory/Datasets/PhatescRNA/p0_labels.csv')
    ...
    plt.title('repulsion and knn and pcs ' + str(repulsion_strength) + ' ' + str(u_knn) + ' ' + str(n_pcs) +' randseed'+ str(p0_random_seed))
    plt.show()
...
def main_mESC(ncomps=100, knn=30, p0_random_seed=12, run_palantir_func=False):
    ...
    for i in [1.0,2,2.5,3.0,4.0,5.0,6.0,7.0,8.0,9.0,10.0,11.0]:
        ...
        print(sub.shape[0])
        np.random.seed(data_random_seed)
        idx_sub= np.random.choice(a=np.arange(0, sub.shape[0]), size=min(n_sub,sub.shape[0]), replace=False, p=None)  # len(true_label)
    ...
    meso_good =  ['CD24', 'FoxA2', 'Oct4',  'CD45', 'Ki67', 'Vimentin', 'Cdx2', 'CD54', 'pStat3-705', 'Sox2', 'Flk-1',
     'Tuj1', 'SSEA1', 'H3K9ac', 'Lin28', 'PDGFRa', 'bCatenin', 'EpCAM', 'CD44', 'GATA4', 'Klf4', 'CCR9', 'p53']
    marker_genes_ecto = ['Oct4','Nestin',   'CD45', 'Vimentin', 'Cdx2', 'Flk-1', 'PDGFRa',  'CD44',
     'GATA4', 'CCR9', 'CD54', 'CD24', 'CD41','Tuji']
    marker_genes_meso_paper_sub = ['Oct4', 'CD54','SSEA1', 'Lin28', 'Cdx2', 'CD45', 'Nanog', 'Sox2', 'Flk-1', 'Tuj1', 'PDGFRa', 'EpCAM', 'CD44', 'CCR9', 'GATA4']

    marker_genes_meso_paper = ['Nestin', 'FoxA2', 'Oct4', 'CD45',   'Sox2', 'Flk-1', 'Tuj1', 'PDGFRa', 'EpCAM', 'CD44', 'GATA4','CCR9','Nanog','Cdx2','Vimentin'] #'Nanog''Cdx2','Vimentin'
    marker_genes_endo = ['Sca-1''Nestin', 'CD45', 'Vimentin', 'Cdx2', 'Flk-1', 'PDGFRa', 'CD44',
                         'GATA4', 'CCR9', 'CD54', 'CD24', 'CD41', 'Oct4']
    marker_genes_meso =  ['Sca-1', 'CD41', 'Nestin', 'Desmin', 'CD24', 'FoxA2', 'Oct4', 'CD45', 'Ki67', 'Vimentin', 'Cdx2', 'Nanog', 'pStat3-705', 'Sox2', 'Flk-1', 'Tuj1', 'H3K9ac', 'Lin28', 'PDGFRa', 'EpCAM', 'CD44', 'GATA4', 'Klf4', 'CCR9', 'p53', 'SSEA1','bCatenin','IdU']#'IdU',

    marker_dict = {'Ecto': marker_genes_ecto, 'Meso': marker_genes_meso,'Endo': marker_genes_meso}
    marker_genes = marker_dict[type_germ]#['Nestin',   'CD45', 'Vimentin', 'Cdx2', 'Flk-1', 'PDGFRa', 'EpCAM', 'CD44',   'GATA4', 'CCR9', 'CD54', 'CD24', 'CD41','Oct4'] #['FoxA2', 'Oct4', 'Nanog', 'Sox2', 'Tuj1']
    ...
    true_label_int =  [i for i in true_label]
    adata.obs['day'] = ['0'+str(i) if i<10 else str(i) for i in true_label_int]
    true_label_str = [str(i)  for i in true_label_int]#the way find_root works is to match any part of root-user to majority truth

    print(adata.obs['day'])
    if normalize == True:
        sc.pp.scale(adata, max_value=5)
        print(colored('normalized','blue'))
    else: print(colored('NOT normalized', 'blue'))
    ...
    np.random.seed(data_random_seed)
    idx = np.random.choice(a=np.arange(0, adata.shape[0]), size=adata.shape[0], replace=False,p=None)  # len(true_label)
...
def main():
    dataset ='EB'#'mESC'#'Human'#,'Toy'#,'Bcell'  # 'Toy'
    if dataset == 'Human':
