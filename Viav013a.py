...
def draw_trajectory_dimred(X_dimred, cluster_labels, super_cluster_labels, super_edgelist,  x_lazy,alpha_teleport, projected_sc_pt, true_label,knn,title_str = "hitting times"):
    ...
    df = pd.DataFrame({'x': x, 'y': y, 'cluster': cluster_labels, 'super_cluster': super_cluster_labels, 'projected_sc_pt': projected_sc_pt},
    ...
    for e_i, (start, end) in enumerate(super_edgelist):
        ...
        if direction_arrow ==1:
            ax2.arrow(xp[closest_loc], smooth[closest_loc],xp[closest_loc+step]-xp[closest_loc], smooth[closest_loc+step]-smooth[closest_loc], shape='full', lw=0, length_includes_head = True, head_width = 1, color = 'black')#, head_starts_at_zero = direction_arrow )
            ...
            ax2.arrow(xp[closest_loc], smooth[closest_loc],xp[closest_loc-step]-xp[closest_loc], smooth[closest_loc-step]-smooth[closest_loc], shape='full', lw=0, length_includes_head = True, head_width = 1, color = 'black')
...
class PARC:
    def __init__(self, data, true_label=None, anndata=None, dist_std_local=2, jac_std_global='median', keep_all_local_dist='auto',
                 too_big_factor=0.4, small_pop=10, jac_weighted_edges=True, knn=30, n_iter_leiden=5, pseudotime= False, root=0, path= '/home/shobi/Trajectory/', seed = 99, super_cluster_labels = False, super_node_degree_list = False, x_lazy=0.95, alpha_teleport = 0.99, root_str = "root_cluster"):
    ...
    def compute_hitting_time(self, sparse_graph, root, x_lazy, alpha_teleport, number_eig=0):
        ...
        eig_val, eig_vec = np.linalg.eig(norm_lap)  # eig_vec[:,i] is eigenvector for eigenvalue eig_val[i] not eigh as this is only for symmetric. the eig vecs are not in decsending order
    ...
    def directed_laplacian2(self, A_bias):
        ...
        for i in range(n):
            ...
            teleport_edge = (np.ones((1,n))-np.identity(n)[i,:])*(deg_val[i]*(1-alpha_teleport))/(alpha_teleport*n)
    ...
    def simulate_markov(self,A):
        if A.shape[0]>42: print('A in markov', A[43,:],A[42,:],A)
        else: print('simulate A',A)
        n_states = A.shape[0]
        ...
        for i in range(num_sim):
            ...
            for state_i in range(P.shape[0]):
                ...
                else: hitting_array_temp[state_i, 0] = first_time_at_statei[0]
            hitting_array = np.append(hitting_array, hitting_array_temp, axis=1)
            if i%10==0: print('hitting temp', hitting_array_temp)
        hitting_array = hitting_array[:, 1:]
        hitting_array=np.mean(hitting_array, axis=1)
        print('hitting from sim markov',hitting_array)
    ...
    def compute_hitting_time_onbias(self, laplacian, inv_sqr_deg,  root, x_lazy, alpha_teleport, number_eig=0):
        ...
        eig_val, eig_vec = np.linalg.eig(laplacian)  # eig_vec[:,i] is eigenvector for eigenvalue eig_val[i] not eigh as this is only for symmetric. the eig vecs are not in decsending order
        print('eig val', eig_val.shape, eig_val)
        if number_eig == 0: number_eig = eig_vec.shape[1]
        print('number of eig vec', number_eig)
        Greens_matrix = np.zeros((N, N), float)
    ...
    def run_subPARC(self):
        ...
        if self.pseudotime == True:
            ...
            edgeweights, edgelist = local_pruning_clustergraph(reweighted_sparse_vc, local_pruning_std=0.0, too_big_factor=self.too_big_factor)#0.5
            ...
            hitting_times_2, roundtrip_times_2 = self.compute_hitting_time_onbias(norm_lap,d_sqrt_inv, root=root, x_lazy=x_lazy,alpha_teleport=alpha_teleport)
            ...
            remove_outliers = hitting_times[hitting_times< np.percentile(hitting_times, 90)]#hitting_times[hitting_times<np.mean(hitting_times)+np.std(hitting_times)]
            ...
            svgpath_local =self.path+"vc_graph_locallytrimmed_Root" + str(root) + "lazy" + str(x_lazy) + 'JacG' + str(self.jac_std_global) + 'toobig'+str(int(self.too_big_factor*100))+"M"+str(len(set(true_labels))) +".svg"
    ...
    def draw_piechart_graph(self,bias = False):
        ...
        for e_i, (start, end) in enumerate(edgelist):
            ...
            ax.add_line(lines.Line2D([node_pos[start,0], node_pos[end,0]], [node_pos[start,1],node_pos[end,1]],color = 'black', lw=edgeweight[e_i]*edgeweight_scale, alpha=0.5))
            ...
            if (node_pos[start,0] < node_pos[end,0]): direction_arrow = 1
            ...
            if direction_arrow == 1:
                ...
                ax.arrow(xp[250], smooth[250], xp[250 + step] - xp[250], smooth[250 + step] - smooth[250], shape='full', lw=0,
        ...
        for e_i, (start, end) in enumerate(edgelist):
            ...
            ax2.add_line(lines.Line2D([node_pos[start,0], node_pos[end,0]], [node_pos[start,1],node_pos[end,1]],color = 'black', lw=edgeweight[e_i]*edgeweight_scale, alpha=0.5))
            z = np.polyfit([node_pos[start,0], node_pos[end,0]],  [node_pos[start,1],node_pos[end,1]],1 )
            minx = np.min(np.array([node_pos[start,0], node_pos[end,0]]))
            ...
            if direction_arrow == 1:
                ...
                ax2.arrow(xp[250], smooth[250], xp[250 + step] - xp[250], smooth[250 + step] - smooth[250], shape='full', lw=0,
                          length_includes_head=True, head_width=arrow_head_w,
                          color='black')
                #ax.plot(xp, smooth, linewidth=edgeweight[e_i], c='pink')
            else:
        ...
        if bias == True: title_pt="Pseudotime on biased edges"
        else:title_pt="Pseudotime on random walk"
...
def main():
    ...
    if dataset.startswith('Toy'):
        ...
        if dataset =="ToyCyclic":
            df_counts = pd.read_csv("/home/shobi/Trajectory/Datasets/ToyCyclic/ToyCyclic_M5_n3000d1000.csv", 'rt', delimiter=",")
            df_ids = pd.read_csv("/home/shobi/Trajectory/Datasets/ToyCyclic/ToyCyclic_M5_n3000d1000_ids.csv", 'rt',delimiter=",")
    ...
    p0 = PARC(adata_counts.obsm['X_pca'][:,0:50], true_label, jac_std_global=2, knn=30, too_big_factor=0.4, pseudotime=True,path = "/home/shobi/Trajectory/Datasets/"+dataset+"/", root = 3, root_str = root_str) #*.4
    p0.run_PARC()
    super_labels = p0.labels

    super_edges = p0.edgelist
    super_pt = p0.scaled_hitting_times #pseudotime pt
    p1 = PARC(adata_counts.obsm['X_pca'][:,0:50], true_label, jac_std_global=1, knn=30, too_big_factor=0.05, path = "/home/shobi/Trajectory/Datasets/"+dataset+"/",pseudotime=True, root =61, super_cluster_labels=super_labels, super_node_degree_list=p0.node_degree_list,root_str = root_str) #*.4
    p1.run_PARC()
    labels = p1.labels
    ...
    print('end tsne')
    draw_trajectory_dimred(embedding, labels, super_labels, super_edges,p1.x_lazy, p1.alpha_teleport, p1.single_cell_pt, true_label,knn=p0.knn, title_str='hitting times: Random walk original')

    draw_trajectory_dimred(embedding, labels, super_labels, super_edges,
                           p1.x_lazy, p1.alpha_teleport, p1.single_cell_pt_bias, true_label, knn=p0.knn, title_str='hitting times: Random Walk on biased edges')
